...
function extractTranslateKey(content) {
  const m = content.match(/\{\{\s*['"]([^'"]+)['"]\s*\|\s*(?:translate|transloco)\b[^}]*\}\}/);
  return m ? m[1] : null;
}
    ...
    try {
      for (const entry of fs.readdirSync(dir, { withFileTypes: true })) {
        ...
        if (entry.isDirectory()) {
          collectJson(full);
        } else if (entry.isFile() && entry.name.endsWith(".json") && !seen.has(full)) {
          seen.add(full);
          results.push(full);
        }
      }
  ...
  function scan(dir, depth) {
    ...
    try {
      for (const entry of fs.readdirSync(dir, { withFileTypes: true })) {
        ...
        if (/^(i18n|locales?|translations?|lang|languages?)$/i.test(entry.name)) {
          collectJson(full);
        } else {
          scan(full, depth + 1);
        }
  ...
  socket.on("message", (raw) => {
    ...
    if (action === "delete") {
      ...
      if (tmatch && tmatch.type !== "self-closing") {
        const contentPat = new RegExp(`${tmatch.opening}>([\\s\\S]*?)</${tmatch.tag}\\s*>`, "s");
            ...
            for (const tFile of tFiles) {
              try {
                ...
                if (getNestedJsonKey(json, translateKey) !== undefined) {
                  saveSnapshot(tFile, path.relative(process.cwd(), tFile), "edit");
                  setNestedJsonKey(json, translateKey, newText);
                  const indentMatch = raw.match(/\n(\s+)"/);
                  const indent = indentMatch ? indentMatch[1].length : 2;
                  fs.writeFileSync(tFile, JSON.stringify(json, null, indent) + "\n", "utf-8");
                  logChange(
                    "edit",
